...
struct_deskriptor  struc                           ;Структура сегментного дескриптора
...
struct_interrupt_deskriptor  struc                           ;Структура дескриптора таблицы прерываний
...
struct_idtr struc                           ;Структура idtr
    limit       dw 0                         ;16-bit table limit
    idtr_low       dw 0                    ;Смещение биты (0-15)
    idtr_hight       dw 0                    ;Смещение биты (31-16)
struct_idtr  ends

;Флаги уровней доступа сегментов	;76543210 : 7--(1-физ память),
					; 6,5--(привелегии),
					; 4,Если этот бит установлен, то дескриптор определяет сегмент кода или данных, а если сброшен, то системный объект (например, сегмент состояния задачи, локальную дескрипторную таблицу, шлюз).
					;3--(1,1-для кода, 1,0-для данных),
					; 2--(0-рост стека вверх для данны,для кода-бит подчинения)
					; 1--(0-процессор только выполняет,нельзя считывать для кода, 1-чтение и запись для данных)
					; 0--(бит обращение к сегменту)
;Если процессор обращался к сегменту для чтения или записи данных или для выполнения кода, размещённых в нём, то бит A будет установлен (равен 1), иначе - сброшен (0). 
...
access_dpl_three       EQU 01100000B           ;X<DPL,DPL>XXXXX - привелегии доступа, доступ может получить любой код
...
;Заполняем дескриптор gdt
 ...
 ;Заполняем дескриптор сегмента стека
  ...
  ;Заполняем дескриптор кода защищенного режима
    ...
    mov [bx][struct_deskriptor.base_medium],dl
    mov [bx][struct_deskriptor.base_hight],dh        
    or  [bx][struct_deskriptor.attribs],40h	 ;40h - 10000000 (работа с физ. памятью) tasm не позволяет писать в bin
...
;Разрешим маскируемые и немаскируемые прерывания
 ...
 ;Переходим к сегменту кода защищенного режима
    db 66h                                      
    retf ;push точка входа в защищенный режим
...
cls  proc near                          ;Процедура очистки консоли
    ...
    mov  ax,1700h	                       ;символ пробела с атрибутом синего цвета ah-цвет al-символ
...
;Сегмент данных реального/защищенного режима
    ...
    irpc    index, 0123456789ABCDEF					;создаем массив из структур struct_interrupt_deskriptor
        idt_0&index struct_interrupt_deskriptor <0, code_for_protected_mode_descriptor, 0, access_trap, 0>        ; 00...0F   (1--адрес обработчика,  2--селектор кода, 3--параметры, 4--уровень доступа, 5--адрес обработчика)
    endm
